...
本 PRD 定义 `orchlet` 的新版产品需求：以现有 `golutra` 为产品功能参考，重建一个现代化、高性能、跨平台的本地优先 AI CLI 多智能体协作工作区。新版必须保留旧版的核心用户价值和关键行为，包括本地工作区管理、多 AI CLI 成员邀请、聊天到终端派发、终端输出回写聊天、多窗口终端、托盘通知、技能管理、设置、诊断和数据持久化；但不要求兼容旧版数据、旧 redb 数据库或旧 `.golutra` 工作区结构。新版可以重做内部架构、模块边界、前后端技术栈和数据层，以获得更好的性能、可维护性、可测试性和长期演进能力。
...
新版的产品成功取决于两个要求同时成立：第一，旧版 `golutra` 中已验证有价值的能力必须通过 capability checklist 被逐项保留、替换或明确放弃；第二，新架构必须避免继续复制旧版中 store 过重、IPC 契约分散、终端状态多事实源、持久化缺少 schema 管理、UI 与副作用耦合等老化点。
...
- **Project Context:** Product rebuild。现有 `golutra` 是功能参考和产品行为样本，`orchlet` 是现代架构版重建；旧数据和旧内部实现不作为兼容约束。
