...
本 UX 规格补足 PRD 与 Epics 中缺少的页面级、按钮级和视觉 parity 细节。参考项目 `/Users/wdx/opc/golutra` 是用户可见 UI、交互流程和功能行为母版；orchlet 新实现使用 React + TypeScript + Tauri 2 + Rust + SQLite/rusqlite + `.orchlet`，但内部架构变化不得改变用户看到的 Golutra 产品形态，除非先获得明确批准并记录为 parity exception。
...
- Golutra parity screens 优先使用参考应用的 `Material Symbols Outlined` 图标、`Be Vietnam Pro` 字体、颜色变量、玻璃面板、窗口壳和响应式布局；不得引入新的视觉语言。
- 所有 icon button 必须提供 `aria-label`/tooltip；图标库选择服从 Golutra parity，非 parity 工具界面才可使用 lucide。
- 不使用 Vue 组件、Pinia store 或旧 `.golutra` 数据文件作为新实现边界；但可用旧 Vue 组件名作为 React parity 屏幕/组件命名映射，方便追踪用户可见结构。

## Golutra Parity Design System

### Visual Tokens

React UI 必须先移植 Golutra 全局视觉基线，再实现业务屏幕：

| Token | Golutra reference | React requirement |
| --- | --- | --- |
| Font | `Be Vietnam Pro` woff2 assets | 作为 app 默认 sans 字体 |
| Icons | `Material Symbols Outlined` woff2 | parity 屏幕默认图标体系 |
| Dark background | `--color-background: 15 15 18` | 默认深色桌面背景 |
| Surface/panel | `--color-surface`、`--color-panel`、`--color-panel-strong` | 用 CSS variables 暴露给 Tailwind/utilities |
| Accent | `--color-primary: 56 189 248` | 主操作、active nav、glow、mention highlight |
| Shell | `.window-frame`、`.titlebar`、`.window-body` | 无边框窗口壳与 titlebar 必须对齐 |
| Panels | `glass-panel`、`glass-modal` | 聊天、侧栏、卡片、弹窗复用 |
| Scrollbar | `.custom-scrollbar` | 所有滚动区统一窄滚动条 |
| xterm | global xterm textarea/viewport overrides | 终端输入、IME、viewport 不得扰动布局 |

### Screen Baselines

每个 parity story 必须产出参考截图和 React 当前截图。至少覆盖：

- Workspace Selection: empty recent、recent grid、more dropdown、open error。
- Main Chat: default channel、DM list、message list、emoji panel、mention dropdown、members sidebar。
- Friends: project/global friends、status menu、invite menu。
- Settings: account/avatar menu、theme cards、language rows、terminal configuration、data maintenance。
- Skill Store: store tab empty、installed/local folder/import card。
- Plugin Marketplace: browse/installed placeholder。
- Terminal Window: empty tabs、single pane with tab、split layouts、tab search、find overlay。
- Notification Preview: unread list、open terminal、ignore/view all footer。

截图差异必须分类为 rendering noise、approved parity exception 或 bug。未经批准的布局、颜色、文案、流程或功能差异都是 release blocker。

### Window And Component Mapping

React 组件应按 Golutra 用户可见结构映射：

| Golutra Vue reference | React parity target |
| --- | --- |
| `App.vue` | `AppShell` + window mode switch |
| `SidebarNav.vue` | `SidebarNav` |
| `WorkspaceSelection.vue` | `WorkspaceSelection` |
| `ChatInterface.vue` | `ChatInterface` |
| `chat/components/*` | `ChatSidebar`、`ChatHeader`、`MessagesList`、`ChatInput`、`MembersSidebar`、member rows |
| `chat/modals/*` | invite/member/rename/roadmap/skill modals |
| `FriendsView.vue` | `FriendsView` |
| `TerminalWorkspace.vue` | `TerminalWorkspace` |
| `TerminalPane.vue` | `TerminalPane` |
| `Settings.vue` | `Settings` |
| `SkillStore.vue` | `SkillStore` |
| `PluginMarketplace.vue` | `PluginMarketplace` |
| `NotificationPreview.vue` | `NotificationPreview` |
| `ToastStack.vue` / `ContextMenuHost.vue` | global toast/context menu hosts |
...
- [ ] React 组件按 Golutra 用户可见屏幕和组件边界拆分，不迁移 Vue/Pinia 内部实现。
- [ ] 每个 parity 屏幕都有参考截图、React 截图、差异分类和人工验收记录。
