...
- Golutra parity screens must match the reference app icon and visual system first, including `Material Symbols Outlined`; `lucide-react` may be used only for non-parity tooling or approved replacements. Icon-only buttons must have `aria-label` and tooltip.
...
UX-DR7: User-visible screens, window modes, flows, copy semantics and visual states must match `/Users/wdx/opc/golutra` unless an explicit parity exception is approved and recorded.
...
FR80: Epic 9 - Golutra UI and behavior parity gate。
...
**FRs covered:** FR80, plus user-visible parity coverage for FR1-FR79.

**Nonfunctional requirements covered:** NFR29, NFR30, NFR31, NFR32, NFR35, NFR37, NFR43, NFR45.

**UX requirements covered:** UX-DR1, UX-DR2, UX-DR3, UX-DR4, UX-DR5, UX-DR6, UX-DR7.

**Implementation notes:** 当前 Epic 1-8 的工程能力可复用，但不能作为产品验收完成依据。Epic 9 是 release blocker。React 组件应按 Golutra 用户可见屏幕边界实现：AppShell/Titlebar/SidebarNav、WorkspaceSelection、ChatInterface、FriendsView、TerminalWorkspace/TerminalPane、Settings、SkillStore、PluginMarketplace、NotificationPreview、ToastStack、ContextMenuHost。Material Symbols、Be Vietnam Pro、Golutra CSS variables、window-frame/titlebar/glass/xterm baseline 必须先进入 React。
...
**Requirements:** FR80, NFR35, NFR45, UX-DR7.

**Acceptance Criteria:**

**Given** the reference app lives at `/Users/wdx/opc/golutra`
**When** parity baseline is prepared
**Then** screen inventory, component mapping, visual tokens and core interaction inventory are documented from source.

**Given** current React UI exists
**When** the baseline compares it to Golutra
**Then** deviations are listed as parity bugs, approved exceptions or not-yet-captured items.

**Given** future parity stories begin
**When** they need screenshots
**Then** baseline artifact defines required reference/current screenshot states and storage paths.
...
**Requirements:** FR8, FR9, FR52, FR65, FR67, FR80, NFR22, NFR30, NFR45, UX-DR3, UX-DR7.

**Acceptance Criteria:**

**Given** the app opens in any window mode
**When** the shell is rendered
**Then** `window-frame`、titlebar、window controls、resize handles、focused/maximized/inactive states match Golutra.

**Given** a workspace is open
**When** the main window is rendered
**Then** SidebarNav, avatar/status menu, unread badge, mobile bottom nav and active tab styling match Golutra.

**Given** a user triggers toasts, context menus, theme or language changes
**When** global systems respond
**Then** ToastStack, ContextMenuHost and cross-window theme/language sync match Golutra behavior and style.
...
**Requirements:** FR1, FR3, FR4, FR5, FR6, FR7, FR8, FR80, NFR13, NFR31, NFR45, UX-DR7.

**Acceptance Criteria:**

**Given** no workspace is open
**When** WorkspaceSelection renders
**Then** open-folder hero card, recent workspace grid, more dropdown/search, empty state, error toast and background treatment match Golutra.

**Given** a directory is opened or a recent workspace is selected
**When** the backend returns success, read-only fallback or registry conflict
**Then** the flow remains functional and the visible states match Golutra where old behavior exists.

**Given** screenshots are captured
**When** reference and React screenshots are compared
**Then** no intentional visual deviation remains without an approved parity exception.

### Story 9.4: ChatInterface parity

As a user,
I want chat to match Golutra's three-panel workbench,
So that conversations, terminal streams and member coordination work the same after the React rewrite.

**Requirements:** FR19-FR32, FR45-FR51, FR80, NFR3, NFR4, NFR29, NFR32, NFR45, UX-DR2, UX-DR4, UX-DR7.

**Acceptance Criteria:**

**Given** the active tab is chat
**When** the main workbench renders
**Then** ChatSidebar, ChatHeader, MessagesList, ChatInput and MembersSidebar match Golutra layout, spacing, icons, colors and responsive behavior.

**Given** users manage conversations
**When** they pin, mute, rename, clear, delete, select channel/DM or inspect unread
**Then** menus and states match Golutra.

**Given** users send and read messages
**When** messages include own bubbles, terminal streams, date separators, mentions, image or roadmap attachments
**Then** rendering and status behavior match Golutra.

**Given** users use input affordances
**When** they open quick prompts, mention suggestions, emoji panel or send/stop states
**Then** interaction and visual behavior match Golutra.

### Story 9.5: Friends、members、invite and modal parity

As a user,
I want friends, members and invite flows to match Golutra,
So that member management does not change during the React migration.

**Requirements:** FR10-FR18, FR20, FR57-FR60, FR61-FR62, FR80, NFR29, NFR31, NFR45, UX-DR7.

**Acceptance Criteria:**

**Given** FriendsView renders
**When** project/global contacts exist or are empty
**Then** sections, friend cards, status dots, action buttons and menu placement match Golutra.

**Given** MembersSidebar renders
**When** members are grouped by owner/admin/assistant/member
**Then** row layout, status display and action menu behavior match Golutra.

**Given** invite or management modals open
**When** users invite admin/assistant/member/friends, manage member, rename conversation, open roadmap or skill modals
**Then** modal layout, copy semantics, controls, placeholder behavior and close/focus behavior match Golutra.

### Story 9.6: TerminalWorkspace and TerminalPane parity

As a user,
I want the terminal window to match Golutra,
So that terminal-heavy workflows keep the same muscle memory.

**Requirements:** FR33-FR44, FR51, FR80, NFR2, NFR4, NFR5, NFR11, NFR22, NFR32, NFR45, UX-DR4, UX-DR7.

**Acceptance Criteria:**

**Given** the terminal window opens
**When** TerminalWorkspace renders
**Then** header, subtitle, tab search, new tab button, recent closed button and tab bar match Golutra.

**Given** tabs and panes are used
**When** users create, close, restore, search, drag, pin or assign tabs to single/split/grid panes
**Then** visual states and behavior match Golutra.

**Given** TerminalPane renders
**When** users type, attach, reconnect, search, clear, select, copy or encounter terminal crash/resource limit
**Then** xterm styling, find overlay, attach/reconnect/fatal overlays and context menu behavior match Golutra.

### Story 9.7: Settings parity

As a user,
I want settings to match Golutra,
So that account, theme, language, terminal and data preferences remain understandable.

**Requirements:** FR63-FR71, FR80, NFR29, NFR30, NFR31, NFR45, UX-DR7.

**Acceptance Criteria:**

**Given** Settings renders
**When** users navigate sections
**Then** settings sidebar, active section styling, content width, separators and scroll behavior match Golutra.

**Given** users edit settings
**When** they use account/avatar menu, theme cards, language rows, members/terminal config, notification toggles, keybinds or data maintenance
**Then** controls, validation, persistence and placeholder boundaries match Golutra.

### Story 9.8: SkillStore and PluginMarketplace parity

As a user,
I want skill and plugin screens to match Golutra,
So that placeholder and local skill behavior stay honest and familiar.

**Requirements:** FR55-FR60, FR80, NFR17, NFR28, NFR45, UX-DR6, UX-DR7.

**Acceptance Criteria:**

**Given** SkillStore renders
**When** users view store/installed tabs, filters, local folders or import card
**Then** header, search, segmented tabs, filters, card grid and empty/placeholder behavior match Golutra.

**Given** PluginMarketplace renders
**When** users view browse/installed tabs
**Then** placeholder cards/actions and disabled/future boundaries match Golutra.

**Given** local skill folder actions run
**When** users import, delete, open, link or unlink skills
**Then** functional behavior remains intact while UI matches Golutra.

### Story 9.9: NotificationPreview and tray flow parity

As a user,
I want notification preview to match Golutra,
So that tray hover/click handling remains a compact desktop preview rather than a normal page.

**Requirements:** FR52-FR54, FR80, NFR6, NFR22, NFR45, UX-DR3, UX-DR7.

**Acceptance Criteria:**

**Given** notification preview opens
**When** unread items exist
**Then** transparent rounded window, glass card, header, unread count, item rows, tags, secondary badges and footer match Golutra.

**Given** users click preview actions
**When** they open conversation, open terminal, open all terminals, ignore all or view all
**Then** action behavior and hover lifecycle match Golutra.
...
**Requirements:** FR79, FR80, NFR20, NFR35, NFR37, NFR43, NFR44, NFR45, UX-DR7.

**Acceptance Criteria:**

**Given** parity release gate runs
**When** all reference screens and core flows are reviewed
**Then** every user-visible item in `docs/rebuild/feature-inventory.md` and `docs/rebuild/parity-checklist.md` is marked pass, approved exception or approved abandonment.

**Given** screenshots and interaction smoke are captured
**When** differences remain
**Then** each difference is classified as rendering noise, approved parity exception or blocking bug.

**Given** release notes are prepared
**When** parity gate completes
**Then** release notes state this build is a React/Tauri internal rebuild of Golutra, not a redesigned app.
